...
function render(data) {
    ...
    const arcPaths = svg.selectAll(".arc")
        ...
        .on("click", function (event, d) {
            ...
            if (!d.data.competencies) return; // If there are no competencies, exit
...
.on("click", function(event, d) {
    ...
    // Reset outer arcs to their original position and radius
    ...
    // Add transition to the outer arcs
    arcPaths.transition()
        .duration(1000)
        .attrTween("d", function(d) {
            const interpolate = d3.interpolateObject({ startAngle: 0, endAngle: 0 }, d);
            return function(t) {
                return arcCategory(interpolate(t));
            };
        });
