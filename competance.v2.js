...
svg.selectAll(".arcCategory")
    ...
    // Add event listener for inner arcs to show/hide outer arcs on click
svg.selectAll(".arcCategory")
.on("click", function (event, d) {
    const category = d.data.category;
    const outerArcs = svg.selectAll(".arcCompetency")
        .filter(d => d.data.category === category);
    const labels = svg.selectAll(".labelCompetency")
        .filter(d => d.data.category === category);

    const isHidden = outerArcs.style("display") === "none";

    outerArcs.style("display", isHidden ? "block" : "none");
...
svg.selectAll(".arcCompetency")
    ...
    .attr("fill", (d, i) => d3.schemeCategory10[i % 10]) // Use modulo to ensure color consistency
    .on("mouseover", function (event, d) {
